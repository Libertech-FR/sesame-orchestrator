...
function rotateHandlerLogIfNeeded(logFile: string, maxSizeBytes: number, maxFiles: number) {
  ...
  try {
    ...
    Logger.error(
      `Could not rotate handler log file ${logFile}: ${(err as Error).message}`,
      rotateHandlerLogIfNeeded.name,
    );
  }
}
...
export function createHandlerLogger(config: ConfigService, handler: string) {
  ...
  try {
    ...
    Logger.error(
      `Failed to create handler logger for <${handler}>: ${(err as Error).message}`,
      createHandlerLogger.name,
    );
  }
