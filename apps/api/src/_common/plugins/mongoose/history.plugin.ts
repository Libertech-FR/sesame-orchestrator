...
function detectChanges<T = Query<any, any>>(
  ...
  const beforeForComparison = JSON.parse(JSON.stringify(before?.toObject ? before.toObject() : before));
  const afterForComparison = JSON.parse(JSON.stringify(after?.toObject ? after.toObject() : after));
...
export function historyPlugin(schema: Schema, options: HistoryPluginOptions) {
  ...
  schema.post('save', async function (after: Document | null) {
    ...
    if (!hasChanged) {
      ...
      logger.debug(
        `No significant changes detected for ${mergedOptions.collectionName} ${after?._id ?? before?._id}, skipping audit log.`,
      );
      return;
    }
