...
export class HealthHistoryService {
  ...
  public async getAdaptiveHistory(): Promise<HealthHistorySnapshot[]> {
    ...
    try {
      const [fiveMinutesSnapshots, hourlySnapshots, dailySnapshots] = await Promise.all([
        ...
        this.readBucketSnapshots(
          HEALTH_HISTORY_1H_INDEX_KEY,
          HEALTH_HISTORY_1H_DATA_KEY,
          oneHourStart,
          fiveMinutesStart - 1,
        ),
        this.readBucketSnapshots(
          HEALTH_HISTORY_1D_INDEX_KEY,
          HEALTH_HISTORY_1D_DATA_KEY,
          oneDayStart,
          oneHourStart - 1,
        ),
      ]);
  ...
  private async writeBucketSnapshot(params: {
    ...
    const { indexKey, dataKey, bucketSizeMs, retentionMs, ttlSeconds, nowTimestamp, payload } = params;
  ...
  private async readBucketSnapshots(
    indexKey: string,
    dataKey: string,
    minScore: number,
    maxScore: number | '+inf',
