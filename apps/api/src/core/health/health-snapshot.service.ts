...
const NATIVE_MEMORY_DERIVE_MIN_GROWTH_MB = resolveThresholdMb('SESAME_HEALTH_NATIVE_DERIVE_MIN_GROWTH_MB', {
  dev: 256,
  prod: 128,
});
const ENABLE_HTTP_GITHUB_CHECK = !/^(false|0|off|no)$/i.test(
  `${process.env.SESAME_HEALTH_ENABLE_HTTP_CHECK || 'false'}`,
);
...
export class HealthSnapshotService {
  ...
  private async checkHttpGithubThrottled(): Promise<HealthIndicatorResult<'http-github'>> {
    ...
    const hasFreshValue = Boolean(
      this.lastHttpGithubIndicator && now - this.lastHttpGithubCheckedAt < HTTP_GITHUB_CHECK_INTERVAL_MS,
    );
  ...
  private buildMemoryNativeIndicator(
    nativeMb: number,
    externalMb: number,
    arrayBuffersMb: number,
  ): {
  ...
  private checkCpu(): Record<
    string,
    { status: 'up' | 'down'; load1mPerCore: number; threshold: number; cores: number }
  > {
    const cpuCount = Math.max(cpus().length, 1);
  ...
  private checkMemoryHeap(): Record<
    string,
    { status: 'up' | 'down'; usedMb: number; thresholdMb: number; usedPercent: number }
  > {
