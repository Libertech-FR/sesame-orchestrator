...
import {
  ...
  AC_INTERNAL_DEFAULT_ROLES_GRANTS,
...
export class RolesService extends AbstractServiceSchema<Roles> {
  ...
  private async validateInherits(roleName: string, inherits: string[]): Promise<void> {
    ...
    const allowed = new Set<string>([AC_ADMIN_ROLE, AC_GUEST_ROLE, ...AC_INTERNAL_DEFAULT_ROLES.map((r) => r.name)]);
    ...
    for (const r of dbRoles) {
      const rName = String((r as any).name ?? '')
        .trim()
        .toLowerCase();
  ...
  public async update<T extends AbstractSchema | Document>(
    ...
    const nextName =
      typeof (update as any)?.name === 'string' ? String((update as any).name) : String((existing as any)?.name ?? '');
  ...
  private getResourcesFromDecorators(): string[] {
    ...
    const wrappers = [...this._discoveryService.getControllers(), ...this._discoveryService.getProviders()];
  ...
  public async getResources(): Promise<Array<{ resource: string }>> {
    ...
    return [...resources].sort((a, b) => a.localeCompare(b)).map((resource) => ({ resource }));
  }
}
