    ...
    for await (const identity of identities) {
      ...
      if (typeof employeeNumber === 'string') {
        ...
        this.mongo
          ...
          .updateOne({ _id: identity._id }, { $set: { 'inetOrgPerson.employeeNumber': [employeeNumber] } });
        ...
        if (typeof employeeNumber[0] === 'number') {
          ...
          this.mongo
            .collection('identities')
            .updateOne({ _id: identity._id }, { $set: { 'inetOrgPerson.employeeNumber': employeeNumber.map(String) } });
        }
      }
    }
