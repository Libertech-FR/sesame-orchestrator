...
describe('PasswordPoliciesDto', () => {
  ...
  it('should reject invalid step daysBefore type', async () => {
    const dto = plainToInstance(PasswordPoliciesDto, {
      passwordExpirationReminderSteps: [{ daysBefore: 'not-a-number', template: 'mail_reminder_30d', subject: 'J-30' }],
    });
  ...
  it('should accept valid usage reminder payload', async () => {
    const dto = plainToInstance(PasswordPoliciesDto, {
      passwordUsageExpirationTtlSeconds: 86400,
      passwordUsageReminderSteps: [{ daysBefore: 7, template: 'mail_pwd_expire_7d', subject: 'J-7' }],
      passwordUsageReminderSubject: 'Sujet usage',
      passwordUsageReminderTemplatesByDays: { '7': 'mail_pwd_expire_7d' },
      passwordUsageReminderSubjectsByDays: { '7': 'J-7' },
    });
