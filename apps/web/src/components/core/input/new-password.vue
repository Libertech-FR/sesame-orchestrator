<template>
  <div>
    <div class="col-12">
      <q-input @update:model-value="checkPassword($event, 'pass')" :label-color="pwdColor" v-model="newPassword" label="Nouveau mot de passe" :type="typePasswordProp">
        <template v-slot:append>
          <q-icon name="mdi-eye" @click="togglePassword" style="cursor: pointer" />
        </template>
      </q-input>
      <q-input
        @update:model-value="checkPassword($event, 'confirm')"
        v-model="confirmNewPassword"
        :label-color="confirmColor"
        label="Confirmation du nouveau mot de passe"
        :type="typeConfirmProp"
      >
        <template v-slot:append>
