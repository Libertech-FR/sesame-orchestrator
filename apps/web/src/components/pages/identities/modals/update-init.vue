...
q-dialog(
  ...
  q-card.identity-modal-card(flat bordered)
    q-card-section.identity-modal-header.row.items-center.no-wrap.bg-info.text-white
      q-avatar(round color="white" text-color="info" icon="mdi-email-arrow-right" size="32px")
      .column.q-ml-md.col
        .text-h6.text-weight-medium Envoyer le mail d'initialisation
        .text-caption(style="opacity: 0.92") Les identités listées recevront un mail d'initialisation
    q-separator
    q-card-section.identity-modal-body
      .identity-modal-body-top
        ...
        .row.items-center.no-wrap.q-mb-sm
          q-icon(color="info" name="mdi-account-multiple-outline" size="22px")
          .text-subtitle1.q-ml-sm.text-weight-medium Destinataires
          q-space
          q-badge(color="info" text-color="white" rounded) {{ selectedRows.length }}
        q-banner(
...
const mainText = computed(
  () =>
    `Vous allez envoyer une invitation à ${selectedRows.value.length} identité${selectedRows.value.length > 1 ? 's' : ''} (statut « ${props.identityTypesName} »). Vérifiez la liste puis confirmez.`,
)

const checkboxLabel = computed(() => {
  return `Envoyer l'invitation à toutes les identités synchronisées du filtre (${props.allIdentitiesCount} identité${props.allIdentitiesCount > 1 ? 's' : ''})`
})
