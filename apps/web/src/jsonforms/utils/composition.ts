import { computeLabel, type ControlElement, type DispatchPropsOfControl, type DispatchPropsOfMultiEnumControl, isDescriptionHidden, type JsonFormsSubStates, type JsonSchema, type UISchemaElement } from '@jsonforms/core'
...
>(
  ...
  const controlWrapper = computed(() => {
    ...
    const id = buildControlElementId(
      input.control.value.id,
      (input.control.value as any)?.key,
    )
  ...
  const elementId = computed(() =>
    buildControlElementId(input.control.value.id),
  )
...
export const buildControlElementId = (
  controlId: string,
  suffixes: string[] = [],
): string => {
  const sanitizeSegment = (segment: string | null | undefined) => {
    ...
    return trimmed.length > 0 ? trimmed.replace(/#\//g, '').replace(/\//g, '_') : undefined
  }

  const segments = [
    sanitizeSegment(controlId),
    ...suffixes.map(sanitizeSegment),
