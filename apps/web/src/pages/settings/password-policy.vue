<template>
  <div>
    ...
    <div class="q-pa-md q-gutter-md">
      ...
      <div row="row">
        <q-toggle dense v-model="hasUpper" color="green" label="Doit contenir au moins une Majuscule" />
      </div>
      <div row="row">
        <q-toggle dense v-model="hasLower" color="blue" label="Doit contenir au moins une Minuscule" />
      </div>
      <div row="row">
        <q-toggle dense v-model="hasNumber" color="orange" label="Doit contenir au moins un chiffre" />
      </div>
      <div row="row">
        <q-toggle dense v-model="hasSpecialChars" color="blue" label="Doit contenir au moins un caractère special" />
      </div>
      <div row="row">
        <q-toggle dense v-model="checkPwned" color="black" label="Vérifier si le mot de pass est connu avec pwned " />
      </div>
      <div row="row">
        <q-toggle dense v-model="smsEnabled" color="red" label="Reinitialisation par SMS active" />
      </div>
      <div class="row">
      ...
      <div class="q-pa-md q-gutter-sm fixed-bottom">
        <q-btn color="primary" style="width: 100%" @click="saveParams">
          <div class="ellipsis">Sauvegarder les parametres</div>
        </q-btn>
      </div>
    </div>
  </div>
...
async function readParams() {
  ...
  const {
    ...
    error,
    refresh,
  } = await useHttp(`/settings/passwdadm/getpolicies`, {
    method: 'GET',
  })
  if (error.value) {
    handleError({
...
async function saveParams() {
  ...
  const {
    ...
    error,
    refresh,
  } = await useHttp(`/settings/passwdadm/setpolicies`, {
    method: 'POST',
    body: data,
  })
  if (error.value) {
