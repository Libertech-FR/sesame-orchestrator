...
q-card.flex.column.fit.absolute(flat)
  q-toolbar.bg-transparent.q-pa-none(style='border-radius: 0;')
    q-btn.icon(stretch icon='mdi-arrow-left' flat @click='navigateToTab(`/settings/roles`)')
    ...
    q-toolbar-title.cursor-pointer(@click='navigateToTab(`/settings/roles/${role._id}`)')
...
export default defineNuxtComponent({
  ...
  async setup() {
    ...
    const {
      ...
      error,
      refresh,
    } = await useHttp<Role>(`/core/roles/` + $route.params._id, {
      method: 'get',
  ...
  computed: {
    ...
    tab: {
      ...
      set(value: string) {
        this.navigateToTab(`/settings/roles/${this.isNew ? NewTargetId : this.role!._id}/${value === 'index' ? '' : value}`)
      },
    },
    tabs() {
      return [
        ...
        },
        {
          ...
          bgColor: 'orange',
          textColor: 'orange',
          action: (i) => this.navigateToTab(`/settings/roles/${this.isNew ? NewTargetId : i._id}/debug`),
