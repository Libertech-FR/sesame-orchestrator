...
export class AgentsController extends AbstractController {
  ...
  public async search(@Res() res: Response, @SearchFilterSchema() searchFilterSchema: FilterSchema, @SearchFilterOptions() searchFilterOptions: FilterOptions): Promise<Response> {
    ...
    const [data, total] = await this._service.findAndCount(searchFilterSchema, AgentsController.projection, searchFilterOptions);
  ...
  public async update(@Param("_id", ObjectIdValidationPipe) _id: Types.ObjectId, @Body() body: AgentsUpdateDto, @Res() res: Response): Promise<Response> {
