import { DeleteResult } from 'mongodb'
import { Test, TestingModule } from '@nestjs/testing'
import { AgentsService } from './agents.service'
import { getModelToken } from '@nestjs/mongoose'
import { Agents } from '~/core/agents/_schemas/agents.schema'
import { Model, Types } from 'mongoose'

describe('AgentsService', () => {
  let service: AgentsService
  let model: Model<Agents>
  const _id = new Types.ObjectId()
  const date = new Date()
  const mockAgents: Agents = {
    _id,
    metadata: {
      createdAt: date,
      createdBy: 'console',
      lastUpdateAt: date,
      lastUpdateBy: 'console',
    },
    info: [],
  }

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AgentsService,
        {
          provide: getModelToken(Agents.name),
          useValue: Model,
        },
      ],
    }).compile()
    service = module.get<AgentsService>(AgentsService)
    model = module.get<Model<Agents>>(getModelToken(Agents.name))
  })

  describe('create', () => {
    it('should create a new record', async () => {
      const createTest = {}
      jest.spyOn(model, 'create').mockImplementationOnce(() => Promise.resolve(mockAgents))
      const result = await service.create(createTest)
      expect(result).toEqual(mockAgents)
    })
  })

  describe('search', () => {
    it('should return an array of records and total count', async () => {
      const expected = [mockAgents]
      const total = 1
      jest.spyOn(model, 'countDocuments').mockResolvedValueOnce(total)
      jest.spyOn(model, 'find').mockResolvedValueOnce(expected)
      const [result, resultTotal] = await service.search()
      expect(result).toEqual(expected)
      expect(resultTotal).toEqual(total)
    })

    it('should return an empty array if no records are found', async () => {
      const expected = []
      const total = 0
      jest.spyOn(model, 'countDocuments').mockResolvedValueOnce(total)
      jest.spyOn(model, 'find').mockResolvedValueOnce(expected)
      const [result, resultTotal] = await service.search()
      expect(result).toEqual(expected)
      expect(resultTotal).toEqual(total)
    })
  })

  describe('read', () => {
    it('should return a Agents record by ID', async () => {
      const expected = mockAgents
      jest.spyOn(model, 'findById').mockResolvedValueOnce(expected)
      const result = await service.read(_id.toString())
      expect(result).toEqual(expected)
    })

    it('should return null if Agents record is not found', async () => {
      const expected = null
      jest.spyOn(model, 'findById').mockResolvedValueOnce(expected)
      const result = await service.read('123')
      expect(result).toEqual(expected)
    })
  })

  describe('update', () => {
    it('should update a Agents record by ID with metadata', async () => {
      const agentsDto = { info: { key: 'updated value' } }
      const newObject = { ...mockAgents }
      const expected = newObject.info.push(agentsDto.info)
      jest.spyOn(model, 'findByIdAndUpdate').mockResolvedValueOnce(expected)
      const result = await service.update(_id.toString(), agentsDto)
      expect(result).toEqual(expected)
    })
  })

  describe('remove', () => {
    it('should remove a Agents record by ID', async () => {
      const id = '123'
      const deleteResult: DeleteResult = { acknowledged: true, deletedCount: 1 }
      jest.spyOn(model, 'deleteOne').mockResolvedValueOnce(deleteResult)
      const result = await service.remove(id)
      expect(result).toEqual(deleteResult)
    })
  })
})
