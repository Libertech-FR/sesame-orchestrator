...
import { KeyringsDto } from './dto/keyrings.dto';
...
describe('KeyringsController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<KeyringsController>(KeyringsController);
    service = module.get<KeyringsService>(KeyringsService);
  });

  describe('search', () => {
    it('should return an array of Keyrings objects and the total count', async () => {
      const req = getMockReq();
      const query = { _id: _id.toString() };
      const limit = 10;
      const skip = 1;
      const expectedResult: [Keyrings[], number] = [[object], 1];
    ...
    it('should return an array of Keyrings objects with default pagination', async () => {
      const req = getMockReq();
      const query = { _id: _id.toString() };
      const limit = 0;
      const skip = 0;
  ...
  describe('read', () => {
    it('should return the data successfully', async () => {
      const expectedResult: Keyrings = object;
  ...
  describe('create', () => {
    it('should return a Keyrings object', async () => {
      const dto: KeyringsDto = { someProp: 'value' };
